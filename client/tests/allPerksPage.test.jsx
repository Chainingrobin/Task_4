...
describe("AllPerks page (Directory)", () => {
  ...
  */
  ...
  test("lists public perks and responds to merchant filtering", async () => {
    // Use the seeded record created in setup to drive a deterministic
    // merchant filter selection and assert the UI updates.
    const seededPerk = global.__TEST_CONTEXT__.seededPerk;
    ...
    // Wait for the baseline card so we know the initial fetch completed.
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });

    // Select the merchant from the dropdown (single combobox on the page).
    const merchantSelect = screen.getByRole("combobox");
    fireEvent.change(merchantSelect, {
      target: { value: seededPerk.merchant },
    });

    // Wait for the filtered results to be rendered.
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });

    // Summary text should still reflect the number of matching perks.
    expect(screen.getByText(/showing/i)).toHaveTextContent("Showing");
  });
});
